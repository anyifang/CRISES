%% P4_PLOTFIG4_PROVINCE_PIECHART
% Portable repository version based on P72_plot_fig4_province_piechart.m.
% 核心升级:
...
% https://www.cpnn.com.cn/news/zngc/202409/t20240929_1739974.html
% 陆上风电方面，含塔筒最低中标单价1699元/千瓦，最高中标单价2380元/千瓦
Cost.CAPEX_onshore  = 0.28 * Cost.Dep_Replace;
% https://cpnn.com.cn/news/baogao2023/202307/W020230707618141034612.pdf
% 全年招标的机组价格基本稳定在 3500-4000 元/千瓦（含塔筒）
Cost.CAPEX_offshore = 0.53 * Cost.Dep_Replace;
% https://iea-pvps.org/wp-content/uploads/2025/10/IEA-PVPS-Task-1-NSR-China-2024.pdf
% table 6 0.75 yuan/W + mounting structures constituted around 7.4% of the initial
% investment cost (table 9) 3.1 yuan/W
Cost.CAPEX_dpv      = 0.13 * Cost.Dep_Replace;
% table 6 0.75 yuan/W + brackets accounted for approximately 8.6% of the total dinitial
% investment cost for ground-mounted PV systems in China 2.5 yuan/W
Cost.CAPEX_upv      = 0.13 * Cost.Dep_Replace;
...
% [Onshore]:  9%-18%  42.5->50 m/s  (Mark Bolinger 2021)
%0.09/(50^2-42.5^2) = 1.3e-4
lambda.onshore  = 1.3e-4;
%50->57m/s  5% -10% 0.05/(57^2-50^2) = 6.7e-5   (Fuchs 2023)
lambda.offshore = 6.7e-5;
% [PV]: Stone, Laurie 2020  5% C3/C4 -> C5    5%  (58 -> 70m/s) *0.85*0.89 = 9m/s
% lambda = 0.05 / (29^2 - 20^2)
% lambda = 0.05 / (49^2 - 40^2)
lambda.dpv      = 1.1e-4;
lambda.upv      = 6.2e-5;   % 假设光伏支架的材料成本敏感度对于分布式和集中式相近
...
% P6-style bar-panel configuration
...
% load(dataFile_P6, 'GenLossGrid', 'TotalEconGrid');  自动 运行 不用load

fprintf('Loading P7 Real CF and Dynamic VRE Data...\n');
CF_On  = readtable(fullfile(dataDir,'CF','onshore_attr_era5_cell_2019.csv'));
CF_Off = readtable(fullfile(dataDir,'CF','offshore_attr_era5_cell_2019.csv'));
CF_Upv = readtable(fullfile(dataDir,'CF','upv_attr_era5_cell_2019.csv'));
priceFile = fullfile(dataDir,'Coastal_Prov_ElecPrice.mat');

[VRE_far, ~, prov_names] = load_and_prep_vre(fullfile(dataDir,'VRE_far_filtered.mat'), 'VRE_far', CF_On, CF_Off, CF_Upv, 'Y2060', priceFile);
[VRE_near, ~, ~] = load_and_prep_vre(fullfile(dataDir,'VRE_near_filtered.mat'), 'VRE_near', CF_On, CF_Off, CF_Upv, 'Y2040', priceFile);
[VRE_hist, ~, ~] = load_and_prep_vre(fullfile(dataDir,'VRE_hist_filtered.mat'), 'VRE_hist', CF_On, CF_Off, CF_Upv, 'Y2025', priceFile);
...
for idx_Mr =  2 
    ...
    Prov_GenLoss_Tech = zeros(nProv, 4, nPoints, nScen, 'single');
    Prov_Cap_Tech  = zeros(nProv, 4, nScen);
    HazardOnly_GenLoss_Tech = zeros(4,nScen);
    HazardOnly_EconLoss_Tech = zeros(4,nScen);
    HistHazard_FutureExposure_EconLoss_Tech = zeros(4,nScen);
        ...
        for is = 1:nScen
            ...
            for ip = 1:nProv
                for t = 1:4
                    ...
                    if any(mask)
                        Prov_GenLoss_Tech(ip, t, ie, is) = sum(gen_loss(mask));
                        Prov_Loss_Tech(ip, t, ie, is) = sum(Total_Loss_Row(mask));  %size(Prov_Loss_Tech) 省份 技术 加固风速 情景
                        if ie == 1, Prov_Cap_Tech(ip, t, is) = sum(cap(mask)); end
                    end
                end
            end

            % Hazard-only counterfactual for the manuscript comparison:
            % apply each historical/future hazard state to the fixed 2025
            % layout, 2025 capacity factors, and 2025 electricity prices.
            % This is evaluated only for no strengthening (Delta v = 0).
            if ie == 1
                V_fix = VRE_hist;
                k_fix = idx_map_hist;
                cap_fix = V_fix.cap_GW;
                cf_fix = V_fix.annual_cf;
                price_fix = V_fix.price;
                gen_fix = zeros(height(V_fix),1);
                asset_fix = zeros(height(V_fix),1);
                masks_fix = {V_fix.tech=="onshore",V_fix.tech=="offshore", ...
                    V_fix.tech=="dpv",V_fix.tech=="upv"};

                m = masks_fix{1};
                gen_fix(m) = cap_fix(m).*dt_on(s_idx,k_fix(m))'.*cf_fix(m)*8.76;
                asset_fix(m) = cap_fix(m).*(p_on_b(s_idx,k_fix(m))'*Cost.CAPEX_onshore + ...
                    p_on_y(s_idx,k_fix(m))'*Cost.CAPEX_onshore*Cost.Yield_Factor);
                m = masks_fix{2};
                gen_fix(m) = cap_fix(m).*dt_off(s_idx,k_fix(m))'.*cf_fix(m)*8.76;
                asset_fix(m) = cap_fix(m).*(p_off_b(s_idx,k_fix(m))'*Cost.CAPEX_offshore + ...
                    p_off_y(s_idx,k_fix(m))'*Cost.CAPEX_offshore*Cost.Yield_Factor);
                m = masks_fix{3};
                gen_fix(m) = cap_fix(m).*dt_dpv(s_idx,k_fix(m))'.*cf_fix(m)*8.76;
                asset_fix(m) = cap_fix(m).*p_dpv_d(s_idx,k_fix(m))'*Cost.CAPEX_dpv;
                m = masks_fix{4};
                gen_fix(m) = cap_fix(m).*dt_upv(s_idx,k_fix(m))'.*cf_fix(m)*8.76;
                asset_fix(m) = cap_fix(m).*p_upv_d(s_idx,k_fix(m))'*Cost.CAPEX_upv;

                econ_fix = gen_fix.*price_fix+asset_fix;
                for t_fix = 1:4
                    m = masks_fix{t_fix};
                    HazardOnly_GenLoss_Tech(t_fix,is) = sum(gen_fix(m));
                    HazardOnly_EconLoss_Tech(t_fix,is) = sum(econ_fix(m));
                end

                % Complementary counterfactual L(E1,H0): retain the
                % exposure state used by this scenario (layout, capacity
                % factors, and prices), but apply the historical hazard.
                gen_hist_hazard = zeros(height(V_cur),1);
                asset_hist_hazard = zeros(height(V_cur),1);
                gen_hist_hazard(idx_on) = cap(idx_on).*dt_on(1,k_map(idx_on))'.*cf(idx_on)*8.76;
                asset_hist_hazard(idx_on) = cap(idx_on).*(p_on_b(1,k_map(idx_on))'*Cost.CAPEX_onshore + ...
                    p_on_y(1,k_map(idx_on))'*Cost.CAPEX_onshore*Cost.Yield_Factor);
                gen_hist_hazard(idx_off_m) = cap(idx_off_m).*dt_off(1,k_map(idx_off_m))'.*cf(idx_off_m)*8.76;
                asset_hist_hazard(idx_off_m) = cap(idx_off_m).*(p_off_b(1,k_map(idx_off_m))'*Cost.CAPEX_offshore + ...
                    p_off_y(1,k_map(idx_off_m))'*Cost.CAPEX_offshore*Cost.Yield_Factor);
                gen_hist_hazard(idx_dpv) = cap(idx_dpv).*dt_dpv(1,k_map(idx_dpv))'.*cf(idx_dpv)*8.76;
                asset_hist_hazard(idx_dpv) = cap(idx_dpv).*p_dpv_d(1,k_map(idx_dpv))'*Cost.CAPEX_dpv;
                gen_hist_hazard(idx_upv) = cap(idx_upv).*dt_upv(1,k_map(idx_upv))'.*cf(idx_upv)*8.76;
                asset_hist_hazard(idx_upv) = cap(idx_upv).*p_upv_d(1,k_map(idx_upv))'*Cost.CAPEX_upv;
                econ_hist_hazard = gen_hist_hazard.*price+asset_hist_hazard;
                scenario_masks = {idx_on,idx_off_m,idx_dpv,idx_upv};
                for t_fix = 1:4
                    m = scenario_masks{t_fix};
                    HistHazard_FutureExposure_EconLoss_Tech(t_fix,is) = ...
                        sum(econ_hist_hazard(m));
                end
            end
        end
    end
    Prov_GenLoss_Tech_p = squeeze(sum(Prov_GenLoss_Tech,1)); % 省份加和
    Prov_GenLoss_Tech_p = squeeze(Prov_GenLoss_Tech_p(:,1,:));  % 选择没有加固的情景

    Prov_Loss_Tech_p = squeeze(sum(Prov_Loss_Tech,1));
    Prov_Loss_Tech_p = squeeze(Prov_Loss_Tech_p(:,1,:));

    HazardOnly_GenLoss = sum(HazardOnly_GenLoss_Tech,1);
    HazardOnly_EconLoss = sum(HazardOnly_EconLoss_Tech,1);
    Combined_GenLoss = sum(Prov_GenLoss_Tech_p,1);
    Combined_EconLoss = sum(Prov_Loss_Tech_p,1);
    HazardOnly_GenFold = HazardOnly_GenLoss/HazardOnly_GenLoss(1);
    HazardOnly_EconFold = HazardOnly_EconLoss/HazardOnly_EconLoss(1);
    Combined_GenFold = Combined_GenLoss/Combined_GenLoss(1);
    Combined_EconFold = Combined_EconLoss/Combined_EconLoss(1);

    T_hazard_only_economic = table(string(scen_names(:)), ...
        HazardOnly_GenLoss(:),HazardOnly_GenFold(:), ...
        HazardOnly_EconLoss(:),HazardOnly_EconFold(:), ...
        Combined_GenLoss(:),Combined_GenFold(:), ...
        Combined_EconLoss(:),Combined_EconFold(:), ...
        'VariableNames',{'Scenario','HazardOnly_Gen_TWh','HazardOnly_Gen_Fold', ...
        'HazardOnly_Econ_BUSD','HazardOnly_Econ_Fold','Combined_Gen_TWh', ...
        'Combined_Gen_Fold','Combined_Econ_BUSD','Combined_Econ_Fold'});
    writetable(T_hazard_only_economic, ...
        fullfile(derivedDir,'Fig4_hazard_only_economic_comparison.csv'));

    % Exact two-factor decomposition of the P4 economic-loss increase:
    % L00=L(E0,H0), L10=L(E0,H1), L01=L(E1,H0), L11=L(E1,H1).
    L00_tech = double(Prov_Loss_Tech_p(:,1));
    % For the historical reference scenario H1 = H0, so L(E0,H1) = L(E0,H0).
    HazardOnly_EconLoss_Tech(:,1) = L00_tech;
    L10_tech = double(HazardOnly_EconLoss_Tech);
    L01_tech = double(HistHazard_FutureExposure_EconLoss_Tech);
    L11_tech = double(Prov_Loss_Tech_p);
    dEcon_H_tech = L10_tech-L00_tech;
    dEcon_E_tech = L01_tech-L00_tech;
    dEcon_I_tech = L11_tech-L10_tech-L01_tech+L00_tech;
    dEcon_Total_tech = L11_tech-L00_tech;

    dEcon_H = sum(dEcon_H_tech,1);
    dEcon_E = sum(dEcon_E_tech,1);
    dEcon_I = sum(dEcon_I_tech,1);
    dEcon_Total = sum(dEcon_Total_tech,1);
    % The historical column is the reference state rather than a change scenario.
    dEcon_H(1) = 0;
    dEcon_E(1) = 0;
    dEcon_I(1) = 0;
    dEcon_Total(1) = 0;
    EconDecompResidual = dEcon_Total-(dEcon_H+dEcon_E+dEcon_I);
    assert(max(abs(EconDecompResidual))<1e-10, ...
        'P4 economic hazard-exposure-interaction decomposition does not close.');

    EconHazard_pct = nan(1,nScen);
    EconExposure_pct = nan(1,nScen);
    EconInteraction_pct = nan(1,nScen);
    futureMask = abs(dEcon_Total)>eps;
    EconHazard_pct(futureMask) = 100*dEcon_H(futureMask)./dEcon_Total(futureMask);
    EconExposure_pct(futureMask) = 100*dEcon_E(futureMask)./dEcon_Total(futureMask);
    EconInteraction_pct(futureMask) = 100*dEcon_I(futureMask)./dEcon_Total(futureMask);

    T_economic_decomposition = table(string(scen_names(:)), ...
        dEcon_H(:),dEcon_E(:),dEcon_I(:),dEcon_Total(:), ...
        EconHazard_pct(:),EconExposure_pct(:),EconInteraction_pct(:), ...
        'VariableNames',{'Scenario','HazardEffect_BUSD','ExposureEffect_BUSD', ...
        'InteractionEffect_BUSD','TotalIncrease_BUSD','Hazard_pct', ...
        'Exposure_pct','Interaction_pct'});
    writetable(T_economic_decomposition, ...
        fullfile(derivedDir,'Fig4_economic_decomposition_percentages.csv'));
    % wind对增长的占比
    increas_loss = Prov_Loss_Tech_p - Prov_Loss_Tech_p(:,1);
    wind_increas_loss =sum(increas_loss(1:2,:),1);
    PV_increas_loss =sum(increas_loss(3:4,:),1);
    RA_WIND_TOINCRASE =  wind_increas_loss ./ (PV_increas_loss+wind_increas_loss);  % 后三个值参考来写abstract里的值
    ...
    for is = 1:nScen
        ...
        for ip = [gd_idx, fj_idx, zj_idx]
            ...
            Prov_C = tot_cap * Cost.CAPEX_offshore * Cost_Factor * CRF / Cost.Dep_Replace;
        ...
        for ip = 1:nProv
            for t = 1:4
                ...
                Prov_C = tot_cap * Cost.(sprintf('CAPEX_%s', tech_name)) * Cost_Factor * CRF  / Cost.Dep_Replace;
    ...
    plot_p6_panel_L(ax1, Prov_GenLoss_Tech_p, 'Power generation loss (TWh yr^-^1)', 'a', ...
        row_d_idxs, col_e_idxs, plot_order_idx_P6, idx_Mr, idx_L_col, tech_colors, scen_names, tech_labels, label_L_inner,[.7 0 0], Style);

    % ====== (b) P6 Econ Loss Bar ======
    ax2 = nexttile(t, 2);
    plot_p6_panel_L(ax2, Prov_Loss_Tech_p, 'Economic loss (Billion USD yr^-^1)', 'b', ...
    ...
    Prov_C_GD = tot_cap_gd * Cost.CAPEX_offshore * Cost_Factor_vec * CRF  / Cost.Dep_Replace;
    Prov_NB_GD = Prov_B_GD - Prov_C_GD;

    % Smooth Curve Calculation
    dv_smooth = linspace(0, 20, 100);
    ...
    C_smooth = tot_cap_gd * Cost.CAPEX_offshore * Cost_Factor_smooth * CRF  / Cost.Dep_Replace;
    ...
    grid(ax4, 'on'); xlim(ax4, [0, 20]); ylim(ax4, [0, max(max_nb_gd*1.8, 1)]);
    ...
    % sum(NB_cat(7,1:3))/sum(NB_cat(7,:));  % Guangdong, Fujian, and Zhejiang, which accounts for 76% of the total benefit
    ...
    exportgraphics(fig, fullfile(figureDir,filename), 'Resolution', 300);
    fprintf('>>> Saved %s <<<\n', filename);
    ...
    if Mr == 2
        ...
        for is = 1:nScen
            ...
            if tot_cap_gd > 0
                ...
                V_final = Ud.offshore + enwind_vec;
                Cost_Factor = lambda.offshore * (V_final.^2 - Ud.offshore^2);
                Prov_C = tot_cap_gd * Cost.CAPEX_offshore * Cost_Factor * CRF  / Cost.Dep_Replace;
                [max_nb, max_idx] = max(Prov_B - Prov_C);
        ...
        % --- Print calculated manuscript checks ---
        gen_total = sum(Prov_GenLoss_Tech_p,1);
        econ_total = sum(Prov_Loss_Tech_p,1);
        gen_far_fold = gen_total(5:7)/gen_total(1);
        econ_far_fold = econ_total(5:7)/econ_total(1);
        fprintf(['Power-generation losses increase %.1f- to %.1f-fold from the historical ' ...
            'baseline of %.2f TWh yr^-1 (Fig. 4a). Economic losses increase %.1f- to ' ...
            '%.1f-fold from %.2f billion USD yr^-1 (Fig. 4b). The offshore-wind share ' ...
            'rises from %.1f%% to %.1f%%-%.1f%%, and Guangdong, Fujian, and Zhejiang ' ...
            'together account for %.1f%%-%.1f%% of future VRE economic loss.\n\n'], ...
            min(gen_far_fold),max(gen_far_fold),gen_total(1), ...
            min(econ_far_fold),max(econ_far_fold),econ_total(1), ...
            hist_off_pct,min(fut_off_pct),max(fut_off_pct), ...
            min(fut_gd_fj_zj_pct),max(fut_gd_fj_zj_pct));
...
settings = struct('equivalentYears',Y_sim,'daysPerYear',360, ...
    'restorationMultiplier',2,'strengtheningIncrement_ms',enwind_vec, ...
    'projectLifeYears',Project_Life,'discountRate',Discount_Rate, ...
    'capacityLayoutYears',[2025 2040 2060],'cost',Cost, ...
    'baselineDesignWindSpeed',Ud,'costScaling',lambda);
save(fullfile(derivedDir,'Fig4_calculated_results.mat'), ...
    'settings','Prov_GenLoss_Tech_p','Prov_Loss_Tech_p', ...
    'Prov_GenLoss_Tech','Prov_Loss_Tech','Prov_Cap_Tech', ...
    'NB_allVRE','NB_3prov','NB_cat','opt_dv_major', ...
    'HazardOnly_GenLoss_Tech','HazardOnly_EconLoss_Tech', ...
    'HistHazard_FutureExposure_EconLoss_Tech', ...
    'HazardOnly_GenLoss','HazardOnly_EconLoss', ...
    'HazardOnly_GenFold','HazardOnly_EconFold', ...
    'Combined_GenLoss','Combined_EconLoss','Combined_GenFold','Combined_EconFold', ...
    'dEcon_H_tech','dEcon_E_tech','dEcon_I_tech','dEcon_Total_tech', ...
    'dEcon_H','dEcon_E','dEcon_I','dEcon_Total','EconDecompResidual', ...
    'EconHazard_pct','EconExposure_pct','EconInteraction_pct', ...
    'T_hazard_only_economic','T_economic_decomposition', ...
    'prov_names','scen_names','RA_WIND_TOINCRASE','-v7.3');
...
% =========================================================================
function [V, S, p_names] = load_and_prep_vre(matPath, varName, CF_On, CF_Off, CF_Upv, yearStr, priceFile)
...
[V, S, p_names] = assign_price_and_get_prov(V, yearStr, priceFile);
end

function [V, S, p_names] = assign_price_and_get_prov(V, yearStr, priceFile)
data = load(priceFile,'res_price');
P_Table = data.res_price;
assert(ismember(yearStr,P_Table.Properties.VariableNames), ...
    'Electricity-price column %s is missing.',yearStr);
price_vec_rmb = P_Table.(yearStr);
prov_names_in_table = string(P_Table.Properties.RowNames);
provinceIds = unique(V.prov_idx(isfinite(V.prov_idx)),'sorted');
nProv = max(provinceIds);
p_names = cell(1,nProv);
price_map_usd = nan(nProv,1);
avg_p_usd = mean(price_vec_rmb,'omitnan')*0.145;
for i = 1:nProv
    row = find(V.prov_idx==i,1);
    if isempty(row), p_names{i}=sprintf('Province %d',i); continue; end
    sName = string(V.prov_name(row));
    p_names{i} = char(sName);
    idx = find(strcmpi(sName,prov_names_in_table),1);
    if isempty(idx), price_map_usd(i)=avg_p_usd;
    else, price_map_usd(i)=price_vec_rmb(idx)*0.145;
    end
end
ids = V.prov_idx;
V.price = repmat(avg_p_usd,height(V),1);
valid = isfinite(ids) & ids>=1 & ids<=nProv;
V.price(valid) = price_map_usd(ids(valid));
S = [];
end

function [fname, ok] = find_result_file(baseDir, enwind, delay)
ok = false; fname = '';
cand = {sprintf('repair_time_distribution_C23_enwind%d_delay%.1f_life25.0yr_fix.mat', enwind, delay), ...
    sprintf('repair_time_distribution_C23_enwind%d_delay%g_life25.0yr_fix.mat', enwind, delay)};
